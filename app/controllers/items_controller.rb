    ...
    if @record.is_journal? || @record.is_newspaper?
      ...
      if @items.present?
        ...
        @items = @items.map do |item|
          ...
          # n.yyyy,m
          when /(\d{1,4})\.(\d{4}),(\d{1,2})/ then "#{$2.rjust(4, "0")}-#{$1.rjust(4, "0")}-#{$3.rjust(4, "0")}"
          # n.yyyy
          when /(\d{1,4})\.(\d{4})/ then "#{$2.rjust(4, "0")}-#{$1.rjust(4, "0")}-0000"
          # yyyy,n
          when /(\d{4}),(\d{1,4})/ then "#{$1.rjust(4, "0")}-#{$2.rjust(4, "0")}-0000"
          # yyyy
  ...
  def augment_locations(items_or_holdings)
    items_or_holdings.each do |ioh|
      if ioh.call_number.present? && ioh.location&.code.present?
        ...
        elsif (ml = mono_location(ioh.call_number, ioh.location.code)).present?
